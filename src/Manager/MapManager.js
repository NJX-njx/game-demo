...
class MapManager {
	...
	async loadRoom(layer, room) {
		...
		try {
			...
			// 交互点：先自动触发后手动触发
			const allInteractions = (data.interactions || []).map((obj, idx) => new Interaction(obj.position, obj.size, obj.type, obj.autoTrigger, { ...obj, __id: idx }));
			this.interactions = [
	...
	/** 获取所有方块的碰撞盒数组 */
	getBlockHitboxes() { return this.blockHitboxes || []; }
	/** 获取所有交互点的碰撞盒数组 */
	getInteractionHitboxes() { return this.interactionHitboxes || []; }
		...
		try {
			...
			for (let i = 0; i < this.interactions.length; i++) {
				...
				if (overlapping) {
					...
					// 手动交互：E 键
					if (!inter.autoTrigger && inputManager.isKeyDown && inputManager.isKeyDown('KeyE')) {
						bus.emit(Events.interaction.trigger, { type: inter.type, event: inter.event, data: inter });
					}
				}
			}
